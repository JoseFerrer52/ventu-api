...
function updateProductValidate(
  createUpdateProductValidation: (data: unknown) => Promise<ValidationSchema>,
...
async function createUpdateProductValidation(
  data: unknown
): Promise<ValidationSchema> {
...
async function createFileUpdateProductValidation(
  file: unknown
): Promise<ValidationSchema> {
  const schema = yup.object().shape({
    mimetype: yup
